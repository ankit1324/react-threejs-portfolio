...
function SplashCursor({
  ...
  useEffect(() => {
    ...
    if (!context || !context.gl || !context.ext || !context.ext.formatRGBA) {
      return;
    }
    ...
    function getWebGLContext(targetCanvas) {
      ...
      let ctx = targetCanvas.getContext("webgl2", params);
      const isWebGL2 = !!ctx;
    ...
    function getSupportedFormat(ctx, internalFormat, format, type) {
      if (!supportRenderTextureFormat(ctx, internalFormat, format, type)) {
        switch (internalFormat) {
          case ctx.R16F:
            return getSupportedFormat(ctx, ctx.RG16F, ctx.RG, type);
          case ctx.RG16F:
            return getSupportedFormat(ctx, ctx.RGBA16F, ctx.RGBA, type);
          default:
            return null;
        }
      }
